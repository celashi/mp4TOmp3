...
def convert_to_mp3(folder, file):
    mp4_file = os.path.join(folder, file)
    mp3_file = os.path.splitext(mp4_file)[0] + '.mp3'
    command = ['ffmpeg', '-i', mp4_file, '-acodec', 'libmp3lame', '-q:a', '2', mp3_file]
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        futures = [executor.submit(convert_to_mp3, folder, file) for file in mp4_files]
